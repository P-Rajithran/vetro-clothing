...
const {setShowSearch,getCartCount,navigate,token,setToken,setCartItems }=useContext(ShopContext);
  ...
  return (
    <div className='flex items-center justify-between py-5 font-medium'>
     ...
     <Link className='w-[max(5%,50px)]'  to='/'><img src={assets.logo} className='w-36' alt="" /></Link>
   ...
   <div className='flex items-center gap-6'>
     <img onClick={()=>setShowSearch(true)} src={assets.search_icon} className='w-5 cursor-pointer' alt="Search Icon"/>
         ...
         <div className='group relative'>
          ...
          <p className='absolute right-[-5px] bottom-[-5px] w-4text-center leading-4 bg-black text-white aspect-square rounded-full text-[-8px]'>{getCartCount()}</p>
          </Link>
          <img onClick={()=>setVisible(true)} src={assets.menu_icon} className='w-5 cursor-pointer sm:hidden' alt="MenuIon" />
    </div>
